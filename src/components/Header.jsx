...
export default function Header({ scrollRef }) {
  ...
  return (
    <>
      <header
        className={`flex items-center gap-3 px-3 bg-s1 border-b z-10 transition-transform duration-300 flex-shrink-0 ${hidden?'-translate-y-full':'translate-y-0'}`}
      ...
      {hidden&&(
        <button
          onClick={()=>{setHidden(false);scrollRef?.current?.scrollTo({top:0,behavior:'smooth'})}}
